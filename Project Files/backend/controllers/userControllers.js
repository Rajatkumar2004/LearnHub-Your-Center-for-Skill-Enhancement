...
const registerController = async (req, res) => {
  try {
    // Only allow admin registration for specific credentials
    if (req.body.type && req.body.type.toLowerCase() === 'admin') {
      if (
        req.body.email !== 'admin@gmail.com' ||
        req.body.password !== 'admin143'
      ) {
        return res.status(403).send({ message: "Admin registration is only allowed for admin@gmail.com with the correct password", success: false });
      }
    }
    ...
    return res
      .status(500)
      .send({ message: "Registration failed", success: false });
  }
};
...
const loginController = async (req, res) => {
  try {
    ...
    // Only allow login as admin for the allowed admin and password
    if (user.type === 'Admin') {
      if (user.email !== 'admin@gmail.com' || req.body.password !== 'admin143') {
        return res.status(403).send({ message: "Admin login not allowed", success: false });
      }
    }
    ...
    console.log(error);
    return res.status(500).send({ message: "Login failed", success: false });
  }
};
...
const enrolledCourseController = async (req, res) => {
  ...
  try {
    ...
    if (!enrolledCourse) {
      ...
      // Encrypt the CVV before saving
      if (coursePayment.cardDetails && coursePayment.cardDetails.cvvcode) {
        const salt = await bcrypt.genSalt(10);
        const hashedCVV = await bcrypt.hash(coursePayment.cardDetails.cvvcode.toString(), salt);
        coursePayment.cardDetails.cvvcode = hashedCVV;
      }
...
const sendCourseContentController = async (req, res) => {
  ...
  try {
    ...
    console.log('Enrolled user:', user); // Debug log
    if (!user) {
      return res.status(404).send({
        success: false,
      ...
      console.log('Returning sections:', course.sections); // Debug log
      return res.status(200).send({
        success: true,
...
const addSectionToCourseController = async (req, res) => {
  try {
    ...
    // Prepare new section object with consistent S_content structure
    let newSection = {
    ...
    await course.save();
    res.status(200).json({ success: true, message: 'Section added successfully', section: newSection });
  } catch (error) {
    console.error(error);
...
const updateCourseController = async (req, res) => {
  try {
    const { courseId } = req.params;
    // Parse form data for course fields
    const {
      C_title,
      C_description,
      C_categories,
    ...
    let sections = [];
    // If only one section, req.body.S_title etc. will be string, else array
    const S_titles = Array.isArray(req.body.S_title) ? req.body.S_title : [req.body.S_title];
    const S_descriptions = Array.isArray(req.body.S_description) ? req.body.S_description : [req.body.S_description];
    // Map uploaded files to sections by order
    const files = req.files || [];
    let fileIdx = 0;
    for (let i = 0; i < S_titles.length; i++) {
      let S_content = null;
      if (files[fileIdx] && files[fileIdx].fieldname === 'S_content') {
        S_content = files[fileIdx].filename;
        fileIdx++;
      } else if (req.body[`S_content_${i}`]) {
        // If no new file, keep old filename if sent
        S_content = req.body[`S_content_${i}`];
      }
      sections.push({
        S_title: S_titles[i],
        S_description: S_descriptions[i],
        S_content,
      });
    }
