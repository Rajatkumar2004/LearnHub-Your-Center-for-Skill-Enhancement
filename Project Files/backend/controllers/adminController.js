...
const deleteUserController = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      res
        .status(200)
        .send({ success: true, message: "User and their enrollments deleted successfully" });
    } else {
    ...
    res
      .status(500)
      .send({ success: false, message: "Failed to delete user and enrollments" });
  }
};

// Add: Get all enrollments with user and course info
...
module.exports = {
  ...
  getAllEnrollmentsController, // <-- export new controller
};
